...
public class DSASignTest {
	...
	public static void main(String[] args) throws Exception {
		...
		Map<String, Object> aKeyMap = DSASignTest.initKey();
		byte[] publicKey = DSASignTest.getPublicKey(aKeyMap);
		byte[] privateKey = DSASignTest.getPrivateKey(aKeyMap);
		System.out.println("公钥:\r" + Base64.encode(publicKey));
		System.out.println("私钥:\r" + Base64.encode(privateKey));

		String aInput = "rsa data ";
		byte[] sign = DSASignTest.sign(aInput.getBytes(), privateKey);
		System.out.println("签名后：" + Base64.encode(sign));
		
		boolean ret = DSASignTest.verify(aInput.getBytes(), publicKey, sign);
