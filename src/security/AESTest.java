...
public class AESTest {
	...
	public static void main(String[] args) throws Exception {
		...
		kg.init(256);//java6支持128,192,256
		SecretKey sk = kg.generateKey();
		byte[] key = sk.getEncoded();
		
		System.out.println("secretKey:" + new String(Base64.encode(sk.getEncoded())));
		
		Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");//参数格式：算法/工作模式/填充模式，不同的算法有不同的工作模式
		//加密
		cipher.init(Cipher.ENCRYPT_MODE, toKey(key));
		byte[] input =cipher.doFinal("AES data".getBytes()); //得到加密后的数据
